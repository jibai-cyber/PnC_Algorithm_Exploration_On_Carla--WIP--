...
try:
    from planning.msg import FrenetPath, SLBoundary, SLBoundaryArray, PathBoundary, STGraph
except ImportError:
    FrenetPath = None
    SLBoundary = None
...
from .plotter_params_loader import load_plotter_params
...
class EgoStatePlotter(Node):
    ...
    def __init__(self):
        ...
        plot_cfg = load_plotter_params()
        self.enable_plotting = bool(plot_cfg['enable_plotting'])
        self.plot_update_rate = float(plot_cfg['plot_update_rate'])
        self.error_history_size = int(plot_cfg['error_history_size'])
