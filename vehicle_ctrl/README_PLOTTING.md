...
### 3. 性能问题
- 调整`plot_update_rate`降低绘图频率
- 考虑在不同机器上运行绘图节点

## 扩展

### 添加新的绘图内容
1. 在控制节点`_update_error_visualization`中添加新数据
2. 在绘图节点`plot_data_callback`中解析新数据
3. 在绘图节点`_init_plot`中添加新的子图
4. 在`_update_plot`中更新新图表

### 自定义消息类型
如需更复杂的消息格式，可以创建自定义ROS消息类型：
```bash
ros2 interface create vehicle_ctrl_msgs/PlotData
```