...
class GlobalPathPlannerNode(Node):
    ...
    def __init__(self):
        ...
        try:
            package_share_directory = get_package_share_directory('planning')
            self.osm_file = os.path.join(package_share_directory, 'resource', 'rviz_map_zitu._fixed.osm')
        except PackageNotFoundError:
            # 如果包未安装，使用相对于当前文件的路径
            current_dir = os.path.dirname(os.path.abspath(__file__))
            # 从 planning/modified_EM_planner/global_path_planner.py 到 planning/resource/
            package_dir = os.path.dirname(os.path.dirname(current_dir))
            self.osm_file = os.path.join(package_dir, 'resource', 'rviz_map_zitu._fixed.osm')
            self.get_logger().warn(f"Package 'planning' not found in ROS index. Using relative path: {self.osm_file}")
        ...
        gpp_cfg = planning_section('global_path_planner')
        self._stopped_since = None
        self._stop_speed_threshold_ms = float(gpp_cfg['stop_speed_threshold_ms'])
        self._stop_duration_sec = float(gpp_cfg['stop_duration_sec'])
        self.k_zeroval = float(gpp_cfg['k_zeroval'])
        self._connection_threshold = float(gpp_cfg['connection_threshold'])
        self._astar_max_iterations = int(gpp_cfg['astar_max_iterations'])
        self._centerline_warn_distance = float(gpp_cfg['centerline_warn_distance'])

        self.waypoints = []
        self.current_waypoint_index = 0
        self.path_resolution = float(gpp_cfg['path_resolution'])
        self.waypoint_display_interval = int(gpp_cfg['waypoint_display_interval'])
    ...
    def build_topology_graph(self):
        ...
        connection_threshold = self._connection_threshold
    ...
    def astar_path_search(self, start_lanelet_id, goal_lanelet_id):
        ...
        visited = set()
        max_iterations = self._astar_max_iterations
        iterations = 0
    ...
    def find_nearest_centerline_point(self, x, y):
        ...
        # 如果距离太远，给出详细警告和诊断信息
        if min_dist > self._centerline_warn_distance:
    ...
    def rosout_callback(self, log: RosLog):
        """当 control_base.py 打印 RViz 风格位姿时，视为已停止并用该位姿作为新起点"""
        try:
            ...
            self.get_logger().info("📍 捕捉到 control_base 停止位姿，已更新起点；请在 RViz2 重新设置新的终点。")
        except Exception:
            pass
