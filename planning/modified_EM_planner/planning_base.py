...
try:
    from planning.msg import FrenetPath, SLBoundary, SLBoundaryArray, PathBoundary
except ImportError:
    FrenetPath = None
    SLBoundary = None
...
def _pb():
    return planning_section('planning_base')
...
def _path_bounds_from_two_elements(
    ...
    if road_left_l is None:
        road_left_l = _pb()['road_left_boundary_l_full']
    if road_right_l is None:
        road_right_l = _pb()['road_right_boundary_l']
...
class PlanningBase(Node):
    ...
    def __init__(self):
        ...
        self._pb = planning_section('planning_base')
        self._lp = planning_section('local_path_planner')
        pb = self._pb

        self.control_dt = float(pb['control_dt'])
        self.point_spacing = float(pb['point_spacing'])
        self.ego_half_width = float(pb['ego_width_default']) / 2.0
        self.vehicle_length = float(pb['vehicle_length'])
        self.vehicle_wheelbase = float(pb['vehicle_wheelbase'])
        self.vehicle_max_steering_angle = float(pb['vehicle_max_steering_angle'])
        ...
        self.REUSE_MAX_TIME = float(pb['reuse_max_time'])
        self.REUSE_MAX_L_DEV = float(pb['reuse_max_l_dev'])
        self.REUSE_OBS_XY_TOL = float(pb['reuse_obs_xy_tol'])
        self.REUSE_MAX_EGO_S_JUMP = float(pb['reuse_max_ego_s_jump'])
        ...
        # 状态变量：用于接收 ref_line_smoother 的 update_index / update_points 更新参考线
    ...
    def _effective_road_left_l(
        ...
        if not has_considered_obs or not borrow_allowed:
            return pb['road_left_boundary_l_ego']
        lo, hi = union_merged
        # TODO: 需要考虑右借道障碍物的影响
        if lo - K_ZERO_VAL <= ego_s <= hi + K_ZERO_VAL:
            return pb['road_left_boundary_l_full']
        return pb['road_left_boundary_l_ego']
    ...
    def _use_default_vehicle_info(self):
        """使用默认车辆信息。"""
        self.ego_half_width = float(self._pb['ego_width_default']) / 2.0
        self.vehicle_length = float(self._pb['vehicle_length_fallback'])
        self.vehicle_wheelbase = float(self._pb['vehicle_wheelbase'])
        self.vehicle_max_steering_angle = float(self._pb['vehicle_max_steering_angle'])
        self.vehicle_info_received = True
    ...
    def _validate_and_resample_speed_profile(
        ...
        if abs(pts[0].t) > K_ZERO_VAL:
            ...
            return False, None
        dt = self._pb['planning_speed_profile_resample_dt_s']
        t_end = self._pb['planning_speed_profile_t_horizon_s']
    ...
    def get_reference_path_segment(self, center_idx: int):
        """
        获取参考线路径段：从 center_idx 前 self._pb['backward_distance'] 到后 self._pb['reference_line_horizon'] 的范围。
        首尾截断，保证在全局路径边界内。
        ...
        backward_distance = self._pb['backward_distance']
        forward_distance = self._pb['reference_line_horizon']
    ...
    def publish_sl_boundaries(
        ...
        if self.latest_objects is not None and len(self.latest_objects.objects) > 0:
            for obj in self.latest_objects.objects:
                ...
                if dist_to_ego > 0.0:
                    ...
                    if proj >= 0.0:
                        # 前方
                        if dist_to_ego > self._pb['reference_line_horizon']:
                            continue
                    else:
                        # 后方
                        if dist_to_ego > self._pb['backward_distance']:
                            continue
                ...
                is_static = spd < self._pb['obs_static_speed_thresh_mps']
        ...
        if self.planning_obstacle_pub is not None and PlanningObstacleArray is not None:
            ...
            pam.t_horizon = float(self._pb['planning_obstacle_t_horizon_s'])
        ...
        for b in (sl_boundaries_list or []):
            ...
            if b.start_l > self._pb['road_left_boundary_l_full'] + K_ZERO_VAL:
                continue
            if b.end_l < self._pb['road_right_boundary_l'] - K_ZERO_VAL:
                continue
            ...
            if sl_center is None:
                continue
            _, obj_l = sl_center
            d_left = abs(float(obj_l) - self._pb['road_left_boundary_l_full'])
            d_right = abs(float(obj_l) - self._pb['road_right_boundary_l'])
    ...
    def _compute_and_publish_path_boundary(
        ...
        for o in obs_info_list:
            ...
            if float(o["end_s"]) + self._pb['borrow_s_pad_rear'] < current_ego_s:
                continue

            # TODO: 这里纵向设计成矩形段落，非常容易导致QP求解失败，需要设计成更合理的梯形段落
            lo = float(o["start_s"]) - self._pb['borrow_s_pad_front']
            hi = float(o["end_s"]) + self._pb['borrow_s_pad_rear']
        ...
        if not self.borrow_judge_flag:
            ...
            and (float(stop_line_s_raw) - float(s_ego_raw)) <= self._pb['judge_distance_before_stop_line'])
        ...
        s_min_eff = 0.0
        s_max_eff = min(
            self._pb['reference_line_horizon'],
            max(0.0, float(accumulated_s[-1])) if len(accumulated_s) > 0 else self._pb['reference_line_horizon']
        )
        if stop_line_s_raw is not None and not borrow_now:
            ...
            if s_stop_ego > K_ZERO_VAL:
                # 一定要加上self._pb['path_boundary_s_resolution']，否则无法包含stop_line在内的规划！！！
                # 调试一大圈之后才发现，因此导致之前stop_line投影一直离奇不稳定，出现一系列问题
                # TODO: 根因尚未明晰，后续排查
                s_max_eff = min(s_max_eff, s_stop_ego + self._pb['path_boundary_s_resolution'])
        s_samples = np.arange(s_min_eff, s_max_eff + K_ZERO_VAL, self._pb['path_boundary_s_resolution'])
        if len(s_samples) == 0:
        ...
        for ego_s in s_samples:
            ...
            if len(union_merged) > 0:
                ...
                rl = self._pb['road_left_boundary_l_ego']
            ...
            path_lower, path_upper = _path_bounds_from_two_elements(
                a_key, b_key, ego_s, obs_info_list,
                road_left_l=rl, road_right_l=self._pb['road_right_boundary_l'],
            )
            ...
            if path_upper is None or path_lower is None:
                ...
                if fallback_lower is not None and fallback_upper is not None:
                    ...
                    path_upper = rl
                    path_lower = self._pb['road_right_boundary_l']

            # 先按道路边界裁剪
            path_lower = max(path_lower, self._pb['road_right_boundary_l'])
            path_upper = min(path_upper, rl)
            shrink = self.ego_half_width + self._pb['ego_lat_buffer']
            adc_lo = path_lower + shrink
            adc_hi = path_upper - shrink
            if adc_hi <= adc_lo:
                if ego_s <= self._pb['block_stop_distance'] and stop_line_s_raw is None:
                    valid = False
                    self.get_logger().error(
                        f"PathBoundary: s={ego_s:.2f} 处 ADC 收缩后无可行走廊且无stop_line，判定 blocked（≤{self._pb['block_stop_distance']:.1f}m 内）"
                    )
                break
        ...
        try:
            ...
            # 车辆参数合法性：wheelbase≈0 会导致 max_kappa 爆炸，LocalPathPlanner 内部会 clamp
            ...
            if can_reuse_qp:
                ...
                planner = LocalPathPlanner(
                    ...
                    max_steering_rate=float(self._pb['max_steering_rate']),
                ...
                sol = planner.solve(
                    ...
                    ddl_init=ddl_init,
                    weights={
                        'w_ref': float(lp['w_ref']),
                        'w_dl': float(lp['w_dl']),
                        'w_ddl': float(lp['w_ddl']),
                        'w_dddl': float(lp['w_dddl']),
                        'w_end_l': float(lp['w_end_l']),
                        'w_end_dl': float(lp['w_end_dl']),
                        'w_end_ddl': float(lp['w_end_ddl']),
                    },
                    init_state_tol=(
                        float(lp['init_tol_l']),
                        float(lp['init_tol_dl']),
                        float(lp['init_tol_ddl']),
                    ),
