...
class RefLineSmootherNode(Node):
    """路径平滑节点"""
    ...
    def __init__(self):
        ...
        rls_cfg = planning_section('ref_line_smoother')
        self.smooth_w1 = float(rls_cfg['smooth_w1'])
        self.smooth_w2 = float(rls_cfg['smooth_w2'])
        self.smooth_w3 = float(rls_cfg['smooth_w3'])
        self.smooth_deviation_limit = float(rls_cfg['smooth_deviation_limit'])
        self.point_spacing = float(rls_cfg['point_spacing'])
        self.look_forward_time_sec = float(rls_cfg['look_forward_time_sec'])
        self.look_forward_short_distance = float(rls_cfg['look_forward_short_distance'])
        self.look_backward_distance = float(rls_cfg['look_backward_distance'])
        self.default_forward_distance = float(rls_cfg['default_forward_distance'])
        self.smooth_interval = float(rls_cfg['smooth_interval'])
        self.overlap_length_ratio = float(rls_cfg['overlap_length_ratio'])
    ...
    def get_local_path_segment(self, waypoints, nearest_idx, last_nearest_idx=-1, velocity=None):
        ...
        overlap_length = forward_distance * self.overlap_length_ratio
